...
debugLog( "keras: " + keras.__version__)
...
class ModelAgent():
    def create_model(self):
        ...
        model.add(keras.layers.Dense(64))
        model.add(keras.layers.Activation('relu'))
        model.add(keras.layers.Dense(self.ACTION_SPACE_SIZE, activation="linear"))# anzahl möglicher output controlls
    ...
    def train(self, terminal_state, step):
        ...
        for index, (current_state, action, reward, new_current_state, done) in enumerate(minibatch):
            ...
            self.model.fit(np.array(X), np.array(y), batch_size=self.MINIBATCH_SIZE, verbose=0, shuffle=False, callbacks=[] if terminal_state else None)
        if terminal_state:
...
class QLearningAgent:
    ...
    def getAction(self, field_info, car_location, game_packet):
        ...
        self.state_now = [target_vector.x, target_vector.y, target_vector.z]
        ...
        if self.should_train:
            ...
            agent.update_replay_memory((self.state_now, self.action, self.step_reward, self.old_state, self.done))
    ...
    def manageEpisodes(self):
        ...
        if self.step == 1:
            ...
            self.done = False
            self.target_location = self.getNearestFullBoost()
...
class MyBot(BaseAgent):
    ...
    def get_output(self, packet: GameTickPacket) -> SimpleControllerState:
        ...
        if target == "boost":
            ...
            action = learningAgent.getAction(field_info, car_location, packet)


        self.renderer.begin_rendering()
        self.renderer.draw_line_3d(car_location, learningAgent.target_location, self.renderer.black())
